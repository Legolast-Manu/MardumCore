/*
* Copyright (C) 2017-2018 MardumCoreProject <https://github.com/MardumCoreProject>
*
...
enum SpellIds
{
    ...
    SPELL_DRUID_MardumCore_FRENZY                     = 210722,
    SPELL_DRUID_MardumCore_FRENZY_DAMAGE              = 210723,
...
#define MardumCore_FRENZY_TARGET "MardumCore_frenzy_target"

// 210722 - MardumCore's Frenzy
class spell_arti_dru_MardumCore_frenzy : public SpellScript
{
    PrepareSpellScript(spell_arti_dru_MardumCore_frenzy);

    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Unit* target = GetHitUnit())
            GetCaster()->Variables.Set(MardumCore_FRENZY_TARGET, target->GetGUID());
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_arti_dru_MardumCore_frenzy::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};

// 210722 - MardumCore's Frenzy
class aura_arti_dru_MardumCore_frenzy : public AuraScript
{
    PrepareAuraScript(aura_arti_dru_MardumCore_frenzy);

    void HandleDummyTick(AuraEffect const* aurEff)
    {
        if (Unit* caster = GetCaster())
            if (Unit* target = ObjectAccessor::GetUnit(*caster, caster->Variables.GetValue<ObjectGuid>(MardumCore_FRENZY_TARGET)))
                caster->CastSpell(target, SPELL_DRUID_MardumCore_FRENZY_DAMAGE, true, NULL, aurEff);
    }

    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* caster = GetCaster())
            caster->Variables.Remove(MardumCore_FRENZY_TARGET);
    }

    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(aura_arti_dru_MardumCore_frenzy::HandleDummyTick, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
        AfterEffectRemove += AuraEffectRemoveFn(aura_arti_dru_MardumCore_frenzy::OnRemove, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
void AddSC_artifact_spell_scripts()
{
    ...
    RegisterSpellAndAuraScriptPair(spell_arti_dru_MardumCore_frenzy, aura_arti_dru_MardumCore_frenzy);
